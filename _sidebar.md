* **Linux**
  * [修改语言](linux/change-lang.md)
  * [启用命令的别名](linux/enable-alias-cmd.md)
  * [安装 Aria2 下载器](linux/install-aria2.md)
  * [安装中文字体](linux/install-chsfonts.md)
  * [安装 Edge 浏览器](linux/install-msedge.md)
  * [安装 VSCode](linux/install-vscode.md)
  * [删除 Gnome 应用](linux/remove-gnome-apps.md)
  * [删除 Snap 软件包](linux/remove-snap.md)
  * [设置时区](linux/local-rtc.md)
  * [移除旧内核](linux/remove-old.md)
  * [配置 SSH](linux/ssh-settings.md)
  * [修改 tty/ssh 显示信息](linux/server-motd.md)
  * [SSH 自动切换中文](linux/ssh-zh_cn.md)
  * [启用 sudo 权限](linux/sudo-user.md)
  * [启用 Tab 补全](linux/tab-completion.md)
  * 
* **Windows**
  * [在登录界面隐藏用户](win/win-hide.md)
  * [删除右键菜单的 AMD 项目](win/remove-amd.md)
  * 
* **Docker**
  * [Docker](docker/docker.md)
  * 
* **软件**
  * [Git 使用说明](soft/git-usage.md)
  * [Git 命令](soft/git-cmd.md)
  * [Python 修改 pip 链接](soft/python-pip.md)
  * [Typecho 去除链接中 index.php](soft/typecho-index.md)
  * 